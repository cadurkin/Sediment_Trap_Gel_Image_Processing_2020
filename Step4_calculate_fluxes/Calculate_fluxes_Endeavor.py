...
EN_img_area_7x = (1388/65000) * (1038/65000)
EN_img_area_20x = (1388/187000) * (1038/187000)
EN_img_area_40x = (1388/375000) * (1038/375000)
EN_img_area_80x = (1388/708000) * (1038/708000)
EN_img_area_115x = (1388/1116000) * (1038/1116000)
...
def add_magnification(data):
    ...
    for x in data.file_name:
        name = x.split('_')
        magnification_list.append(name[2])
...
def Particle_flux(data,time, particle_type=None,img_area_7x=EN_img_area_7x, img_area_20x=EN_img_area_20x,img_area_40x=EN_img_area_40x,img_area_80x = EN_img_area_80x, img_area_115x=EN_img_area_115x,bins=bins, bin_mids=bin_mids, bin_width=bin_width):
    data_7x = data[data.magnification == '7x']
    data_20x = data[data.magnification == '20x']
    data_40x = data[data.magnification == '40x']
    data_80x = data[data.magnification == '80x']
    data_115x = data[data.magnification == '115x']

    fov_7x = int(np.max(data_7x.fov))
    fov_20x = int(np.max(data_20x.fov))
    fov_40x = int(np.max(data_40x.fov))
    try:
        fov_80x = int(np.max(data_80x.fov))
    except:
    ...
    if particle_type != None:
        ...
        data_40x = data[data.magnification == '40x']
        try:
            if isinstance(fov_80x, int):
                data_80x = data[data.magnification == '80x']
        except:
            if isinstance(fov_115x, int):
                data_115x = data[data.magnification == '115x']
    ...
    histo_40x = np.histogram(data_40x.ESD,bins=bins)
    flux_40x = histo_40x[0]/fov_40x/img_area_40x/time  #in unit of particles per square meter per day
    flux_40x_error = np.sqrt(histo_40x[0])/fov_40x/img_area_40x/time
    number_counted_40x = histo_40x[0]
    result_40x = pd.DataFrame(np.stack((flux_40x,flux_40x_error,number_counted_40x),-1),columns=('flux_40x','flux_error_40x','number_counted_40x'))

    try:
        histo_80x = np.histogram(data_80x.ESD,bins=bins)
        flux_80x = histo_80x[0]/fov_80x/img_area_80x/time  #in unit of particles per square meter per day
        flux_80x_error = np.sqrt(histo_80x[0])/fov_80x/img_area_80x/time
        number_counted_80x = histo_80x[0]
        result_80x = pd.DataFrame(np.stack((flux_80x,flux_80x_error,number_counted_80x),-1),columns=('flux_80x','flux_error_80x','number_counted_80x'))
    ...
    result['flux_40x'] =  flux_40x
    result['flux_error_40x'] =  flux_40x_error
    result['number_counted_40x'] =  number_counted_40x
    try:
        result['flux_80x'] =  flux_80x
        result['flux_error_80x'] =  flux_80x_error
        result['number_counted_80x'] =  number_counted_80x
    except:
...
plt.loglog(data_flux_allMags.bin_mids[data_flux_allMags.flux_40x!=0],data_flux_allMags.flux_40x[data_flux_allMags.flux_40x!=0]/data_flux_allMags.bin_width[data_flux_allMags.flux_40x!=0], lw=2,marker = 'o',ms=3,color = 'orange', label ='40x')
try:
	...
	plt.loglog(data_flux_allMags.bin_mids[data_flux_allMags.flux_80x!=0],data_flux_allMags.flux_80x[data_flux_allMags.flux_80x!=0]/data_flux_allMags.bin_width[data_flux_allMags.flux_80x!=0], lw=2,marker = 'o',ms=3,color = 'red', label ='80x')
...
def flux_combined(data,threshold_7x, threshold_20x, threshold_40x, threshold_80_or_115x):
    ...
    subset_40xa = data.loc[data.bin_mids >= threshold_40x]
    subset_40x = subset_40xa.loc[subset_40xa.bin_mids < threshold_20x]
    subset_80_or_115xa = data.loc[data.bin_mids >= threshold_80_or_115x]
    subset_80_or_115x = subset_80_or_115xa.loc[subset_80_or_115xa.bin_mids < threshold_40x]

    PSD_flux_all= pd.DataFrame()
    try:
        flux_all = np.concatenate((subset_80_or_115x.flux_80x,subset_40x.flux_40x,subset_20x.flux_20x,subset_7x.flux_7x))
    except:
        flux_all = np.concatenate((subset_80_or_115x.flux_115x,subset_40x.flux_40x,subset_20x.flux_20x,subset_7x.flux_7x))
    try:
        counts_all = np.concatenate((subset_80_or_115x.number_counted_80x,subset_40x.number_counted_40x,subset_20x.number_counted_20x,subset_7x.number_counted_7x))
    except:
        counts_all = np.concatenate((subset_80_or_115x.number_counted_115x,subset_40x.number_counted_40x,subset_20x.number_counted_20x,subset_7x.number_counted_7x))
    try:
        flux_error_all = np.concatenate((subset_80_or_115x.flux_error_80x,subset_40x.flux_error_40x,subset_20x.flux_error_20x,subset_7x.flux_error_7x))
    except:
        flux_error_all = np.concatenate((subset_80_or_115x.flux_error_115x,subset_40x.flux_error_40x,subset_20x.flux_error_20x,subset_7x.flux_error_7x))
    bin_mids_all = np.concatenate((subset_80_or_115x.bin_mids,subset_40x.bin_mids,subset_20x.bin_mids,subset_7x.bin_mids))
    bin_width_all = np.concatenate((subset_80_or_115x.bin_width,subset_40x.bin_width,subset_20x.bin_width,subset_7x.bin_width))
...
data_flux_combined = flux_combined(data_flux_allMags,437,154,109,11)
...
data_agg_flux_combined = flux_combined(data_agg_flux_allMags,437,154,109,11)
data_long_flux_allMags = Particle_flux(data_sinking,time,'long_fecal_pellet')
data_long_flux_combined = flux_combined(data_long_flux_allMags,437,154,109,11)
data_large_flux_allMags = Particle_flux(data_sinking,time,'large_loose_pellet')
data_large_flux_combined = flux_combined(data_large_flux_allMags,437,154,109,11)
data_short_flux_allMags = Particle_flux(data_sinking,time,'short_pellet')
data_short_flux_combined = flux_combined(data_short_flux_allMags,437,154,109,11)
data_mini_flux_allMags = Particle_flux(data_sinking,time,'mini_pellet')
data_mini_flux_combined = flux_combined(data_mini_flux_allMags,437,154,109,11)
data_dense_flux_allMags = Particle_flux(data_sinking,time,'dense_detritus')
data_dense_flux_combined = flux_combined(data_dense_flux_allMags,437,154,109,11)
data_phyto_flux_allMags = Particle_flux(data_sinking,time,'phytoplankton')
data_phyto_flux_combined = flux_combined(data_phyto_flux_allMags,437,154,109,11)
data_foraminifera_flux_allMags = Particle_flux(data_sinking,time,'foraminifera')
data_foraminifera_flux_combined = flux_combined(data_foraminifera_flux_allMags,437,154,109,11)
data_rhizaria_flux_allMags = Particle_flux(data_sinking,time,'rhizaria')
data_rhizaria_flux_combined = flux_combined(data_rhizaria_flux_allMags,437,154,109,11)
data_copepod_flux_allMags = Particle_flux(data,time,'copepod')
data_copepod_flux_combined = flux_combined(data_copepod_flux_allMags,437,154,109,11)
data_amphipod_flux_allMags = Particle_flux(data,time,'amphipod')
data_amphipod_flux_combined = flux_combined(data_amphipod_flux_allMags,437,154,109,11)
data_zooplankton_flux_allMags = Particle_flux(data,time,'zooplankton')
data_zooplankton_flux_combined = flux_combined(data_zooplankton_flux_allMags,437,154,109,11)
data_zooplankton_part_flux_allMags = Particle_flux(data,time,'zooplankton_part')
data_zooplankton_part_flux_combined = flux_combined(data_zooplankton_part_flux_allMags,437,154,109,11)
data_fiber_flux_allMags = Particle_flux(data,time,'fiber')
data_fiber_flux_combined = flux_combined(data_fiber_flux_allMags,437,154,109,11)
data_unidentifiable_flux_allMags = Particle_flux(data,time,'unidentifiable')
data_unidentifiable_flux_combined = flux_combined(data_unidentifiable_flux_allMags,437,154,109,11)
data_pteropod_flux_allMags = Particle_flux(data,time,'pteropod')
data_pteropod_flux_combined = flux_combined(data_pteropod_flux_allMags,437,154,109,11)
data_salp_pellet_flux_allMags = Particle_flux(data,time,'salp_pellet')
data_salp_pellet_flux_combined = flux_combined(data_salp_pellet_flux_allMags,437,154,109,11)
...
data_all_flux_data=data_all_flux_data.merge(data_salp_pellet_flux_combined,on='bin_mids',how='left')
...
data_salp_top = data_salp_pellet_flux_combined.flux/data_flux_combined.flux
...
ax1.bar(data_mids,data_salp_top,bottom=data_agg_top+data_dense_top, width=data_flux_combined.bin_width, color = 'orange',label='salp fp')
ax1.bar(data_mids,data_large_top,bottom=data_salp_top+data_agg_top+data_dense_top, width=data_flux_combined.bin_width, color = 'crimson',label='large loose fp')
