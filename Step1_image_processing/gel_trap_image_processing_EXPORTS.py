...
#set the scale of image pixels per micron
...
for file in os.listdir(directory):
    ...
    if magnification is 7:
    	markers[photo_grey_nobg >= 58] = 1
    	markers[photo_grey_nobg <= 30] = 1
    	markers[photo_brown_nobg > 5] = 1
    if magnification is 20:
    	markers[photo_grey_nobg >= 58] = 1
    	markers[photo_grey_nobg <= 30] = 1
    	markers[photo_brown_nobg > 10] = 1
    if magnification is 50:
    	markers[photo_grey_nobg >= 56] = 1
    	markers[photo_grey_nobg <= 33] = 1
    	markers[photo_brown_nobg >= 9] = 1
    if magnification is 115:
        markers[photo_grey_nobg >= 60] = 1
        markers[photo_grey_nobg <= 40] = 1
        markers[photo_brown_nobg >=18] = 1
    ...
    for b in field_data.Number:
        ...
        for x in np.unique(potential_duplicates2.focal_plane):
            ...
            if len(fp_x.Number) > 1:                                                   #find which ones are in the same focal plane
                ...
                selected_duplicate_index = np.argmin(Area_difference)
        ...
        in_focus_particle = np.argmax(duplicates.edge_intensity)
...
for field in Deduplicated_data.fov.unique():
    ...
    io.imsave(os.path.join(out_directory , 'Measured_Particles_' + str(magnification) + 'x_Mask' + str(int(field)) + '.tiff'), dedup_particles_mask,plugin= 'pil')
