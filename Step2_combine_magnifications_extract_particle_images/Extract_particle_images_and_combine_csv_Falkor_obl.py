...
#Falkor oblique lighting samples
...
#the image processing directory contains subfolders for each magnification that is "./Sample_obl/Sample_7x_obl_mask/Measured_particles_7x.csv"
#where Sample is the name of the sample and a subfolder exists for each magnification.

#The gel image directory contains subfolder for each sample: "./Sample/Sample_obl/Sample_7x_obl/" for each magnification where "Sample"=sample name
...
directory_7x = Sample + '_7x_obl_mask'
directory_20x = Sample + '_20x_obl_mask'
directory_50x = Sample + '_50x_obl_mask'
directory_115x = Sample + '_115x_obl_mask'
os.mkdir(os.path.join(directory,Sample+'_obl',Sample + '_obl_particle_images'))

file_7x = pd.read_csv(os.path.join(directory,Sample+'_obl',directory_7x,'Measured_particles_7x.csv'))
file_20x = pd.read_csv(os.path.join(directory,Sample+'_obl',directory_20x,'Measured_particles_20x.csv'))
file_50x = pd.read_csv(os.path.join(directory,Sample+'_obl',directory_50x,'Measured_particles_50x.csv'))
file_115x = pd.read_csv(os.path.join(directory,Sample+'_obl',directory_115x,'Measured_particles_115x.csv'))
...
for x in np.unique(subset_7x.file_name):
	photo = io.imread(os.path.join(image_directory,Sample, Sample+'_obl',Sample+'_7x_obl',x), plugin= 'pil')
	...
	for p in photo_subset_7x.index:
		...
		io.imsave(os.path.join(directory,Sample+'_obl',Sample + '_obl_particle_images',particle_name),particle)
		
for x in np.unique(subset_20x.file_name):
	photo = io.imread(os.path.join(image_directory,Sample,Sample+'_obl',Sample+'_20x_obl',x), plugin= 'pil')
...
for x in np.unique(subset_50x.file_name):
	photo = io.imread(os.path.join(image_directory,Sample,Sample+'_obl',Sample+'_50x_obl',x), plugin= 'pil')
...
for x in np.unique(subset_115x.file_name):
	photo = io.imread(os.path.join(image_directory,Sample,Sample+'_obl',Sample+'_115x_obl',x), plugin= 'pil')
...
Measured_particles_allMagnifications.to_csv(os.path.join(directory,Sample+'_obl','Measured_particles_all-magnifications.csv'))
