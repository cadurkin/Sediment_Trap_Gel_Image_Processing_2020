...
directory_50x = Sample + '_50x_mask'
...
file_50x = pd.read_csv(os.path.join(directory,Sample,directory_50x,'Measured_particles_50x.csv'))
...
for x in np.unique(subset_7x.file_name):
	...
	for p in photo_subset_7x.index:
		...
		particle_name1 = x.split('.tiff')
		particle_name = particle_name1[0] + '_' + str(int(photo_subset_7x.loc[p].Number)) + '.tiff'
...
for x in np.unique(subset_20x.file_name):
	...
	for p in photo_subset_20x.index:
		...
		particle_name1 = x.split('.tiff')
		particle_name = particle_name1[0] + '_' + str(int(photo_subset_20x.loc[p].Number)) + '.tiff'
		io.imsave(os.path.join(directory,Sample,Sample + '_particle_images',particle_name),particle)
...
for x in np.unique(subset_50x.file_name):
	...
	for p in photo_subset_50x.index:
		...
		particle_name1 = x.split('.tiff')
		particle_name = particle_name1[0] + '_' + str(int(photo_subset_50x.loc[p].Number)) + '.tiff'
		io.imsave(os.path.join(directory,Sample,Sample + '_particle_images',particle_name),particle)
...
for x in np.unique(subset_115x.file_name):
	...
	for p in photo_subset_115x.index:
		...
		particle_name1 = x.split('.tiff')
		particle_name = particle_name1[0] + '_' + str(int(photo_subset_115x.loc[p].Number)) + '.tiff'
		io.imsave(os.path.join(directory,Sample,Sample + '_particle_images',particle_name),particle)
...
for x in Measured_particles_allMagnifications.index:
	...
	filename1=filename.split('.tiff')
	particle_image_name = filename1[0] + '_' + str(int(Measured_particles_allMagnifications.loc[x].Number)) + '.tiff'
