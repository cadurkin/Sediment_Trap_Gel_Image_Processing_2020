...
#Endeavor EN581 brightfield lighting samples
...
#80x image size range: >=10 um ESD
...
directory_80x = Sample + '_80x_mask'
...
file_80x = pd.read_csv(os.path.join(directory,Sample,directory_80x,'Measured_particles_80x.csv'))

subset_7x = file_7x.loc[file_7x.ESD >=256]
subset_20x = file_20x.loc[file_20x.ESD >=64]
subset_40x = file_40x.loc[file_40x.ESD >=45]
subset_80x = file_80x.loc[file_80x.ESD >=11]
...
for x in np.unique(subset_80x.file_name):
	photo = io.imread(os.path.join(image_directory,Sample,Sample+'_80x_images',x), plugin= 'pil')
	photo_subset_80x = subset_80x.loc[subset_80x.file_name == x]
	for p in photo_subset_80x.index:
		particle = photo[int(photo_subset_80x.loc[p].Top):int(photo_subset_80x.loc[p].Bottom),int(photo_subset_80x.loc[p].Left):int(photo_subset_80x.loc[p].Right)]
		particle_name1 = x.split('.jpg')
		particle_name = particle_name1[0] + '_' + str(int(photo_subset_80x.loc[p].Number)) + '.jpg'
		io.imsave(os.path.join(directory,Sample,Sample + '_particle_images',particle_name),particle)
		
		
Measured_particles_allMagnifications = pd.concat([subset_7x,subset_20x,subset_40x,subset_80x])
